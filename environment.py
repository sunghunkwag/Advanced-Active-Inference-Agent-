import torch
import numpy as np

import random

class PixelEnv:
    """
    A simple environment that generates image-like observations.
    The environment consists of a 2D canvas where a shape (e.g., a square) moves.
    For meta-learning, the dynamics (action mappings) can be changed by setting a new task.
    This enhanced version adds visual noise and action stochasticity for more robust learning.
    """
    def __init__(self, size=64, shape_size=5, num_actions=4, num_tasks=4,
                 noise_level=0.1, action_stochasticity=0.15):
        self.size = size
        self.shape_size = shape_size
        self.num_actions = num_actions
        self.pos = np.array([size // 2, size // 2])
        self.goal_pos = np.array([size - shape_size, size - shape_size])
        self.noise_level = noise_level
        self.action_stochasticity = action_stochasticity

        # Define a set of possible tasks (action mappings)
        self._tasks = self._create_tasks(num_actions, num_tasks)
        self.task_id = 0
        self.action_map = self._tasks[self.task_id]

    def _create_tasks(self, num_actions, num_tasks):
        base_map = {
            0: np.array([-1, 0]),  # Up
            1: np.array([1, 0]),   # Down
            2: np.array([0, -1]),  # Left
            3: np.array([0, 1]),   # Right
        }

        tasks = [base_map]
        available_mappings = list(base_map.values())

        for _ in range(num_tasks - 1):
            shuffled_mappings = available_mappings[:]
            random.shuffle(shuffled_mappings)
            new_task = {i: shuffled_mappings[i] for i in range(num_actions)}
            tasks.append(new_task)

        return tasks

    def reset_task(self, task_id=None):
        """
        Sets a new task for the agent to adapt to.
        If task_id is None, a random task is chosen.
        """
        if task_id is None:
            self.task_id = random.randint(0, len(self._tasks) - 1)
        else:
            self.task_id = task_id % len(self._tasks)

        self.action_map = self._tasks[self.task_id]
        return self.task_id

    def _generate_observation(self):
        """
        Generates a 1xHxW tensor representing the current state.
        The image shows the agent (white) and the goal (grey).
        """
        canvas = torch.zeros(1, self.size, self.size)

        # Draw the goal (grey square)
        gx_start = self.goal_pos[0] - self.shape_size // 2
        gx_end = gx_start + self.shape_size
        gy_start = self.goal_pos[1] - self.shape_size // 2
        gy_end = gy_start + self.shape_size
        gx_start, gx_end = max(0, gx_start), min(self.size, gx_end)
        gy_start, gy_end = max(0, gy_start), min(self.size, gy_end)
        canvas[:, gx_start:gx_end, gy_start:gy_end] = 0.5 # Grey color for goal

        # Draw the agent (white square)
        ax_start = self.pos[0] - self.shape_size // 2
        ax_end = ax_start + self.shape_size
        ay_start = self.pos[1] - self.shape_size // 2
        ay_end = ay_start + self.shape_size
        ax_start, ax_end = max(0, ax_start), min(self.size, ax_end)
        ay_start, ay_end = max(0, ay_start), min(self.size, ay_end)
        canvas[:, ax_start:ax_end, ay_start:ay_end] = 1.0 # White color for agent

        # Add visual noise
        if self.noise_level > 0:
            noise = torch.rand_like(canvas) * self.noise_level
            canvas = torch.clamp(canvas + noise, 0, 1)

        return canvas

    def reset(self):
        """
        Resets the agent and goal to random positions.
        """
        self.pos = np.random.randint(0, self.size, size=2)
        self.goal_pos = np.random.randint(0, self.size, size=2)

        # Ensure agent and goal are not at the same spot initially
        while np.linalg.norm(self.pos - self.goal_pos) < self.shape_size:
            self.pos = np.random.randint(0, self.size, size=2)

        return self._generate_observation()

    def step(self, action):
        """
        Takes an action, updates position, and calculates reward.
        Returns: (observation, reward, done, info) tuple.
        """
        prev_dist = np.linalg.norm(self.pos - self.goal_pos)

        # Action Stochasticity
        if random.random() < self.action_stochasticity:
            action = random.randint(0, self.num_actions - 1)

        if action in self.action_map:
            self.pos += self.action_map[action]

        # Clamp position to be within the canvas boundaries
        self.pos = np.clip(self.pos, 0, self.size - 1)
        
        # --- Calculate Reward ---
        current_dist = np.linalg.norm(self.pos - self.goal_pos)
        # Reward is the negative change in distance (closer is better)
        reward = prev_dist - current_dist

        done = False
        if current_dist < self.shape_size:
            reward += 10.0  # Bonus for reaching the goal
            done = True

        return self._generate_observation(), reward, done, {}

    def get_num_actions(self):
        return self.num_actions
